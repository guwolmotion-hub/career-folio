...
function motionCards() {
  ...
  return `<div class="motion-result"><span role="status">${list.length}개 작품${motionState.query ? ' · 검색 결과' : ''}</span><span>제작 · 수강생 / 교육·지도 포트폴리오</span></div><div class="motion-grid">${list.slice(0,motionState.limit).map(motionCard).join('')}</div>${list.length === 0 ? '<p class="motion-empty">검색 결과가 없습니다. 다른 제목으로 검색해 보세요.</p>' : ''}${list.length > motionState.limit ? `<div class="motion-more"><button class="button secondary" data-motion-more>작품 더 보기 <span>${Math.min(motionState.limit,list.length)} / ${list.length}</span></button></div>` : ''}`;
}
...
function motionDetail(id) {
  ...
  if (!item) return '<a class="back-link" href="#archive">← 포트폴리오 목록</a><h1>작품을 찾을 수 없습니다.</h1>';
