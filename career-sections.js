...
function projectVideoWorks(p) {
  ...
  return '<section class="project-video-works" aria-label="프로젝트 영상">'+videos.map(v=>`<article class="project-video-work"><h2>${e(v.title)}</h2><div class="motion-player ${v.format==='short'?'personal-short-player':''}"><iframe src="https://www.youtube-nocookie.com/embed/${e(v.id)}" title="${e(v.title)} 영상 재생" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="motion-player-footer"><span>영상이 재생되지 않으면 YouTube에서 확인해 주세요.</span><a class="text-link" href="${e(safeUrl(v.url))}" target="_blank" rel="noopener noreferrer">YouTube에서 보기 ↗</a></div></article>`).join('')+'</section>';
}
